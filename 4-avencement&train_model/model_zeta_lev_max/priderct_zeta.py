...
def load_models_and_predict():
    ...
    try:
        ...
        details = pd.read_csv('data_son_test_detail.csv')
        summary = pd.read_csv('data_test_summary.csv')
        ...
        # 4. Préparer les données d'entrée (en respectant l'ordre des features du modèle)
        X_slice = details[[
...
def display_results(results):
    ...
    print(results['details'][['id', 'titre', 'Danger%_pred', 'moy_danger_pred']].head())
    
    # Affichage des prédictions pour les fichiers
    print("\n=== PRÉDICTIONS POUR LES FICHIERS ===")
    print(results['summary'][['titre', 'danger_max_pred', 'danger_moy_pred', 'danger_std_pred']].head())
    ...
    plt.subplot(1, 2, 1)
    plt.bar(['Danger%', 'moy_danger'], 
            [results['details']['Danger%_pred'].mean(), 
             results['details']['moy_danger_pred'].mean()])
    plt.title("Prédictions moyennes (tranches)")
    plt.ylabel("Valeur prédite")
    plt.grid(True)
    
    plt.subplot(1, 2, 2)
    plt.bar(['danger_max', 'danger_moy', 'danger_std'],
            [results['summary']['danger_max_pred'].mean(),
             results['summary']['danger_moy_pred'].mean(),
             results['summary']['danger_std_pred'].mean()])
    plt.title("Prédictions moyennes (fichiers)")
    plt.grid(True)
